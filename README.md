...
- Config data, as designed in Figma: supported banks, per-bank field mapping with unmapped-field alerts, FX rates with the ECB's daily feed, a manual refresh and the stored rates by date and source, and the email template editor; off-boarding approvals arrive with client accounts (#98, #100–#102, N3)
...
- KYC review for Compliance: every document waiting on them with its client, counts across the top that also choose what the table shows, and a client's whole pack on one screen with the decision panel under it. A reason has to be typed before Approve or Reject will go, and the client's KYC badge follows the decision
